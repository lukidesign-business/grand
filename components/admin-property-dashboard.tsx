        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(19rem,0.6fr)]">
          <form onSubmit={save} className={`border border-line-soft bg-surface p-6 transition-all duration-500 sm:p-8 ${editingId ? 'ring-1 ring-gold/70 shadow-[0_0_0_1px_rgba(205,171,89,0.2),0_18px_60px_rgba(0,0,0,0.22)]' : ''}`}>
            ...
            <div className="mt-7 grid gap-5 sm:grid-cols-2">
              <label className="text-xs uppercase tracking-wide text-muted">Property name<input value={form.name} onChange={(e) => update('name', e.target.value)} className="admin-input" /></label>
              ...
              <label className="text-xs uppercase tracking-wide text-muted sm:col-span-2">Description<textarea value={form.description} onChange={(e) => update('description', e.target.value)} rows={4} className="admin-input" /></label>
            </div>
            <label onDragOver={dragOver} onDragLeave={dragLeave} onDrop={(event) => handleDrop(event, 'cover')} className="mt-6 block cursor-pointer border border-dashed border-line-strong p-5 text-sm text-muted transition-colors hover:border-gold hover:bg-gold/5"><span className="block text-cream">Cover image</span><span className="mt-1 block text-xs">Click to choose or drag and drop one image here</span><input type="file" accept="image/*" onChange={(event) => { setUploadKind('cover'); upload(event, 'cover') }} className="sr-only" />{uploading && uploadKind === 'cover' ? <span className="mt-2 block text-gold">Uploading…</span> : null}</label>
            <label onDragOver={dragOver} onDragLeave={dragLeave} onDrop={(event) => handleDrop(event, 'gallery')} className="mt-4 block cursor-pointer border border-dashed border-line-strong p-5 text-sm text-muted transition-colors hover:border-gold hover:bg-gold/5"><span className="block text-cream">Gallery images</span><span className="mt-1 block text-xs">Click to choose or drag and drop multiple images here</span><input type="file" accept="image/*" multiple onChange={(event) => { setUploadKind('gallery'); upload(event, 'gallery') }} className="sr-only" />{uploading && uploadKind === 'gallery' ? <span className="mt-2 block text-gold">Uploading…</span> : null}</label>
